...
Confrontando invece il numero di vittorie del ranking aggregato rispetto alle baseline, Model Centrality riesce a trovare un modello che riesce a vincere 29 volte rispetto ad una selezione casuale. Questo significa che su 39 dataset, 29 volte Model Selection è risultato più utile rispetto ad una selezione casuale. Contro il modello che ha le performance medie migliori, in questo caso è stato KNN, Model Centrality ha vinto 23 volte. Non un risultato incoraggiante, considerando che vengono allenati molti modelli per poi ottenere delle performance comparabili ad un unico modello (KNN). La situazione migliorerebbe se si andasse a considerare soltanto Model Centrality.
...
Per ovviare a questa problematica si è deciso portare in produzione una Dashboard con PowerBI, in modo da notificare gli operatori i momenti di anomalia e ricevere feedback di conseguenza sulla precisione del modello.
